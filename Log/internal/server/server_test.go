...
import (
	...
	"github.com/gavinc95/proglog/internal/log"
...
func setupTest(t *testing.T, fn func(*Config)) (client api.LogClient, config *Config, teardown func()) {
	...
	// 0 port is used when we don't care what port we run - automatically assigns a free port
	l, err := net.Listen("tcp", ":0")
	require.NoError(t, err)

	clientOptions := []grpc.DialOption{grpc.WithInsecure()}
	cc, err := grpc.Dial(l.Addr().String(), clientOptions...) // creates the gRPC client connection
	require.NoError(t, err)
...
func testConsumePastBoundary(t *testing.T, client api.LogClient, config *Config) {
	...
	if consume != nil {
		t.Fatal("consume not nil, when we were expecting it to be!")
	}
	got := grpc.Code(err)
